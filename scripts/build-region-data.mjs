...
const LIMIT_RULES = [
  ...
  [/^(\d+) ([a-z]+(?: [a-z]+)*) per day$/i, (m) => `每日 ${m[1]} 条${speciesList(m[2])}`],
...
const DRAWN_IDS = [
  { region: "1", water: "Adam and Eve Rivers", area: /^$/, id: "r1-adam-eve" },
  { region: "1", water: "Campbell River", area: /downstream of the confluence with the Quinsam River$/i, id: "r1-campbell-lower" },
  { region: "1", water: "Campbell River", area: /Quinsam River to the fishing boundary sign/i, id: "r1-campbell-quinsam-maple" },
  { region: "1", water: "Cayeghle River", area: /Colonial River/i, id: "r1-cayeghle" },
  { region: "1", water: "Chemainus River", area: /^$/, id: "r1-chemainus" },
  { region: "1", water: "Cluxewe River", area: /^$/, id: "r1-cluxewe" },
  { region: "1", water: "Conuma River", area: /^$/, id: "r1-conuma" },
  {
    region: "1",
    water: "Courtenay River (the waters below the confluence of the Puntledge and Tsolum Rivers)",
    area: /^$/,
    id: "r1-courtenay",
  },
  { region: "1", water: "Cowichan River", area: /66 Mile Trestle/i, id: "r1-cowichan-upper" },
  { region: "1", water: "Cowichan River", area: /Sandy Pool Regional Park, downstream/i, id: "r1-cowichan-lower" },
  { region: "1", water: "Goldstream River", area: /^$/, id: "r1-goldstream" },
  { region: "1", water: "Koksilah River", area: /tributaries/i, id: "r1-koksilah" },
  { region: "1", water: "Little Qualicum River", area: /all open portions/i, id: "r1-little-qualicum-open" },
  { region: "1", water: "Little Qualicum River", area: /all waters unless indicated/i, id: "r1-little-qualicum-all" },
  { region: "1", water: "Nahwitti River", area: /^$/, id: "r1-nahwitti" },
  { region: "1", water: "Nanaimo River", area: /including tributaries/i, id: "r1-nanaimo-all" },
  { region: "1", water: "Nanaimo River", area: /Highway 19 Bridge/i, id: "r1-nanaimo-cedar-hwy19" },
  { region: "1", water: "Nanaimo River", area: /Highway 1 overpass/i, id: "r1-nanaimo-cedar-hwy1" },
  { region: "1", water: "Nitinat River", area: /^upstream of Parker Creek/i, id: "r1-nitinat-upper" },
  { region: "1", water: "Nitinat River", area: /^downstream of Parker Creek/i, id: "r1-nitinat-lower" },
  { region: "1", water: "Puntledge River", area: /Stotan Falls/i, id: "r1-puntledge-lower" },
  { region: "1", water: "Puntledge River", area: /Morrison Creek closure/i, id: "r1-puntledge-morrison" },
  { region: "1", water: "Qualicum River", area: /All open portions/i, id: "r1-qualicum-open" },
  { region: "1", water: "Qualicum River", area: /upstream of the Big Qualicum Hatchery upper weir/i, id: "r1-qualicum-upper" },
  { region: "1", water: "Quatse River", area: /^$/, id: "r1-quatse" },
  { region: "1", water: "Quinsam River", area: /^$/, id: "r1-quinsam" },
  { region: "1", water: "Reay Creek", area: /^$/, id: "r1-reay" },
  { region: "1", water: "San Juan River", area: /downstream of the Fleet River/i, id: "r1-san-juan-lower" },
  { region: "1", water: "Shawnigan Creek", area: /^$/, id: "r1-shawnigan" },
  { region: "1", water: "Somass River", area: /^$/, id: "r1-somass" },
  { region: "1", water: "Somass River", area: /^Except in those areas/i, id: "r1-somass-open" },
  { region: "1", water: "Somass River", area: /Paper Mill Dam/i, id: "r1-somass-papermill" },
  { region: "1", water: "Somass River", area: /Somass Park/i, id: "r1-somass-park" },
  { region: "1", water: "Sproat River", area: /^$/, id: "r1-sproat" },
  { region: "1", water: "Sproat River", area: /from Sproat Lake/i, id: "r1-sproat-hwy4" },
  { region: "1", water: "Stamp River", area: /^Except in those areas/i, id: "r1-stamp-open" },
  { region: "1", water: "Stamp River", area: /confluence of the Ash River/i, id: "r1-stamp-upper" },
  { region: "1", water: "Stamp River", area: /Stamp Falls fishway/i, id: "r1-stamp-falls" },
  { region: "1", water: "Stamp River", area: /powerline crossing downstream/i, id: "r1-stamp-powerline" },
  { region: "1", water: "Stamp River", area: /Girl Guide Falls/i, id: "r1-stamp-lower" },
  { region: "1", water: "Washlawlis River", area: /^$/, id: "r1-washlawlis" },
  { region: "1", water: "Waukwaas River", area: /^$/, id: "r1-waukwaas" },
...
// Hashes the regulation content rather than the page, so a reworded footer or a
// new navigation link does not read as a change to the rules.
function fingerprint(html) {
  const { rows } = parseTable(html);
  const payload = JSON.stringify({ rows, notes: parseNotes(html) });
  return createHash("sha256").update(payload).digest("hex").slice(0, 16);
}

function serialize(built, modified, fingerprints) {
  const lines = [
    ...
    ...Object.entries(modified).map(([id, date]) => `  ${quote(id)}: ${quote(date)},`),
    "};",
    "",
    ...
    ...Object.entries(fingerprints).map(([id, hash]) => `  ${quote(id)}: ${quote(hash)},`),
    "};",
    "",
...
const built = [];
const modified = {};
// Region 2 is fetched too, even though it is not generated: its page carries
// the date shown for that region, and hashing its parsed rows is what tells us
// DFO has edited the one table a human has to transcribe.
const fingerprints = {};
for (const region of regions) {
  const html = await fetchRegion(region.slug, { refresh });
  modified[region.id] = parseDateModified(html);
  if (HAND_WRITTEN.has(region.id)) {
    fingerprints[region.id] = fingerprint(html);
    continue;
  }
...
for (const { region, spots, notes } of built) {
  ...
  console.log(
    `region${region.id} ${region.en}: ${spots.length} waters, ${rules} rules, ` +
      `${notes.length} region notes, DFO modified ${modified[region.id]}`,
  );
}
for (const id of HAND_WRITTEN) {
  console.log(`region${id} (hand written): DFO modified ${modified[id]}, table hash ${fingerprints[id]}`);
}
...
if (!reportOnly) {
  ...
  writeFileSync(target, serialize(built, modified, fingerprints), "utf8");

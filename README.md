...
- 底图默认使用高清卫星影像，便于查看真实河道和进入路线，可随时切回街道图
- 地图右上角可一键铺满整个浏览器窗口，按 Esc 或再点一次退出
...
- 「今天」按 BC 本地时区（`America/Vancouver`）判断，与 DFO 表格的日期口径
  一致。服务端在 UTC 运行，若按运行机器时区计算，BC 下午 5 点之后会提前跳到
  第二天的规定。
